...
int ObjectRef::l_native_punch(lua_State *L) {
    ...
    float wear = n_punch(sao, puncher, time_from_last_punch, toolcap, dir);
...
int ObjectRef::l_native_get_inventory(lua_State *L)
{
	...
	InvRef inv = ObjectRef::n_get_inventory(sao);
	if (inv.exists())
		inv.push(L);
	else
		lua_pushnil(L);
...
int ObjectRef::l_native_get_wield_list(lua_State *L) {
    ...
    std::string wieldList = n_get_wield_list(sao);
...
int ObjectRef::l_native_get_wielded_item(lua_State *L) {
    ...
    ItemStack selected_item = nativeModApiObjectRef::n_get_wielded_item(sao);
    LuaItemStack::create(L, selected_item);
    return 1;
}
...
int ObjectRef::l_native_get_armor_groups(lua_State *L)
{
	...
	std::map<std::string, int> armorGroups = native_get_armor_groups(sao);
...
int ObjectRef::l_native_set_animation(lua_State *L)
{
    ...
    n_set_animation(sao, frame_range, frame_speed, frame_blend, frame_loop);
    return 0;
}
...
int ObjectRef::l_native_set_local_animation(lua_State *L)
{
	...
	n_set_local_animation(player, frames, frame_speed);
